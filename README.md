...
本仓库用途：作为Farmer-RL v1.0的基座模型，用于继续预训练与微调等。
农业领域大语言模型的 **基座** 工程：用于 **继续预训练（CPT）**、**指令微调（SFT）**（含 **LoRA**）以及本地推理与部署。本仓库提供目录约定、依赖与最小运行脚本；**不包含**模型权重本身，可前往Hugging Face下载模型权重（见下表）。
